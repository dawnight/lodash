...
describe('keys methods', () => {
  lodashStable.each(['keys', 'keysIn'], (methodName) => {
    ...
    it(`\`_.${methodName}\` should ${isKeys ? 'not ' : ''}include inherited string keyed properties`, () => {
      function Foo() {
        this.a = 1;
      }
      Foo.prototype.b = 2;

      let expected = isKeys ? ['a'] : ['a', 'b'],
    ...
    it(`\`_.${methodName}\` should ${isKeys ? 'not ' : ''}include inherited string keyed properties of arrays`, () => {
      arrayProto.a = 1;

      let expected = isKeys ? ['0'] : ['0', 'a'],
        actual = func([1]).sort();
    ...
    it(`\`_.${methodName}\` should ${isKeys ? 'not ' : ''}include inherited string keyed properties of \`arguments\` objects`, () => {
      let values = [args, strictArgs],
    ...
    it(`\`_.${methodName}\` should ${isKeys ? 'not ' : ''}include inherited string keyed properties of string objects`, () => {
      stringProto.a = 1;
