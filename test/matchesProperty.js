...
describe('matchesProperty', () => {
  ...
  it('should preserve the sign of `0`', () => {
    ...
    const actual = lodashStable.map(props, (key, index) => {
      let matches = matchesProperty(key, values[index]),
        pair = pairs[index];
  ...
  it('should coerce `path` to a string', () => {
    ...
    let object = { 'null': 1, 'undefined': 2, 'fn': 3, '[object Object]': 4 },
  ...
  it('should work with a function for `srcValue`', () => {
    ...
    let objects = [{ 'a': { 'a': 1 } }, { 'a': { 'a': 1, 'b': Foo.b, 'c': 3 } }],
  ...
  it('should work with a non-plain `srcValue`', () => {
    ...
    let object = new Foo({ 'a': new Foo({ 'b': 1, 'c': 2 }) }),
