...
describe('slice', () => {
  ...
  it('should work with a `start` >= `length`', () => {
    lodashStable.each([3, 4, Math.pow(2, 32), Infinity], (start) => {
  ...
  it('should work with a `end` >= `length`', () => {
    lodashStable.each([3, 4, Math.pow(2, 32), Infinity], (end) => {
  ...
  it('should treat falsey `end` values, except `undefined`, as `0`', () => {
    ...
    const actual = lodashStable.map(falsey, (end, index) => index ? slice(array, 0, end) : slice(array, 0));
