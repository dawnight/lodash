...
describe('isNative', () => {
  it('should return `true` for native methods', () => {
    let values = [Array, body && body.cloneNode, create, root.encodeURI, Promise, slice, Uint8Array],
  ...
  it('should return `false` for non-native methods', () => {
    ...
    const actual = lodashStable.map(falsey, (value, index) => index ? isNative(value) : isNative());
