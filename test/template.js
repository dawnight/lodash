...
describe('template', () => {
  ...
  it('should support "evaluate" delimiters with single line comments (test production builds)', () => {
    let compiled = template('<% // A code comment. %><% if (value) { %>yap<% } else { %>nope<% } %>'),
      data = { 'value': true };
  ...
  it('should support referencing variables declared in "evaluate" delimiters from other delimiters', () => {
    let compiled = template('<% var b = a; %><%= b.value %>'),
      data = { 'a': { 'value': 1 } };
  ...
  it('should support "interpolate" delimiters containing ternary operators', () => {
    let compiled = template('<%= value ? value : "b" %>'),
      data = { 'value': 'a' };
  ...
  it('should use a `with` statement by default', () => {
    let compiled = template('<%= index %><%= collection[index] %><% _.each(collection, function(value, index) { %><%= index %><% }); %>'),
  ...
  it('should use `_.templateSettings.imports._.templateSettings`', () => {
    let lodash = templateSettings.imports._,
  ...
  it('should tokenize delimiters', () => {
    let compiled = template('<span class="icon-<%= type %>2"></span>'),
      data = { 'type': 1 };
  ...
  it('should support recursive calls', () => {
    let compiled = template('<%= a %><% a = _.template(c)(obj) %><%= a %>'),
      data = { 'a': 'A', 'b': 'B', 'c': '<%= b %>' };
  ...
  it('should coerce `text` to a string', () => {
    let object = { 'toString': lodashStable.constant('<%= a %>') },
      data = { 'a': 1 };
